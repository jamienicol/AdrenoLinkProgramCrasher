...
void main ()
{
  ...
  if (transform_is_axis_aligned_15) {
    ...
    bvec4 tmpvar_41;
    tmpvar_41 = notEqual ((tmpvar_31 & ivec4(1, 2, 4, 8)), ivec4(0, 0, 0, 0));
    vec4 tmpvar_42;
  ...
  vec2 tmpvar_50;
  tmpvar_50 = ((vi_local_pos_27 - tmpvar_7.xy) / tmpvar_7.zw);
  float tmpvar_51;
  if (((tmpvar_32 & 1) != 0)) {
    tmpvar_51 = 1.0;
  } else {
  ...
  v_perspective = tmpvar_51;
  v_op = tmpvar_13.x;
  highp vec2 inv_texture_size_52;
  ivec2 tmpvar_53;
  ...
  highp int tmpvar_55;
  tmpvar_55 = (tmpvar_13.y + 2);
  ivec2 tmpvar_56;
  tmpvar_56.x = int((uint(tmpvar_55) % 1024u));
  tmpvar_56.y = int((uint(tmpvar_55) / 1024u));
  vec4 tmpvar_57;
  tmpvar_57 = mix (mix (texelFetch (sGpuCache, tmpvar_56, 0), texelFetch (sGpuCache, (tmpvar_56 + ivec2(1, 0)), 0), tmpvar_50.x), mix (texelFetch (sGpuCache, (tmpvar_56 + ivec2(2, 0)), 0), texelFetch (sGpuCache, (tmpvar_56 + ivec2(3, 0)), 0), tmpvar_50.x), tmpvar_50.y);
  vec4 tmpvar_58;
  ...
  highp vec2 inv_texture_size_59;
  ivec2 tmpvar_60;
  ...
  highp int tmpvar_62;
  tmpvar_62 = (tmpvar_13.z + 2);
  ivec2 tmpvar_63;
  tmpvar_63.x = int((uint(tmpvar_62) % 1024u));
  tmpvar_63.y = int((uint(tmpvar_62) / 1024u));
  vec4 tmpvar_64;
  tmpvar_64 = mix (mix (texelFetch (sGpuCache, tmpvar_63, 0), texelFetch (sGpuCache, (tmpvar_63 + ivec2(1, 0)), 0), tmpvar_50.x), mix (texelFetch (sGpuCache, (tmpvar_63 + ivec2(2, 0)), 0), texelFetch (sGpuCache, (tmpvar_63 + ivec2(3, 0)), 0), tmpvar_50.x), tmpvar_50.y);
  vec4 tmpvar_65;
  ...
  ) * inv_texture_size_59) * mix (vi_world_pos_28.w, 1.0, tmpvar_51));
